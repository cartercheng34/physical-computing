...
public:
	...
	virtual void Initialize_Simulation_Data()
	{
		...
		case 4:{
			real length=(real)1;int width=4*2;int height=6*2;real step=length/(real)width;
			TriangleMesh<d> cloth_mesh;
			Build_Cloth_Mesh(width,height,step,&cloth_mesh,0,2);
			int n=(int)cloth_mesh.Vertices().size();
			Array<Vector2i> edges;Get_Mesh_Edges(cloth_mesh,edges);
			...
			for(int i=0;i<n;i++){
				soft_body.particles.X(i)=cloth_mesh.Vertices()[i];
				if (i == (width*height/2 - width/2) || i == (width*height/2 - width/2)-1 || i == (width*(height/2+1) - width/2) || i == (width*(height/2+1) - width/2) -1)
					soft_body.particles.M(i)=(real)10;
				else soft_body.particles.M(i)=(real)1;
			...
			soft_body.Set_Boundary_Node(0);
			soft_body.Set_Boundary_Node(width-1);
			soft_body.Set_Boundary_Node(width*height-1);
			soft_body.Set_Boundary_Node(width*height-width);
